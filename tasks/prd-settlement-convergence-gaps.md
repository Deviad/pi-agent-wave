...
- **Gates.** Full serial Node suite: 446 tests, 435 pass, 0 fail, 11 skipped. Bun package gates: 45 pass, 0 fail. `npm run typecheck` clean, `git diff --check` clean, installation rehearsal 1 pass (no worker dispatched). The cleanup and unlaunched-settlement files were also run from `extensions/pi-agent-wave` to confirm they no longer depend on the starting directory. `npm publish --dry-run --json --ignore-scripts` exits 0 with 69 files and no `test/`, `agent-output/` or absolute paths in the artifact, and the three changed source files are inside it.

**Live-path check.** Every automated proof of user story 1 builds the tool through a fake `registerTool` and a fake `exec`, and the installation rehearsal loads the real Pi module loader without dispatching a turn, so the last untested link was a real Pi process reaching the new branch. It does: a `pi --print` run (0.84.1, working-tree package registered, `--no-builtin-tools --tools delegate_graph`, `DELEGATE_GRAPH_DB` pointed at a temporary database outside the repository) initialised a research graph and then collected its never-dispatched operation, reporting `settled: true`, `recorded: "failed"`, the never-started reason and the retained diagnostic path. Verified against the database rather than trusting the model's report: the operation became `failed` with `finished_at` set and `classifier_reason: worker-never-launched`, the run moved from `active` to `awaiting_user`, and the diagnostic landed at mode 600. The live database and its WAL were byte-identical before and after (sha `af89813625e0dbe8`; 63 runs, 302 operations, 1933 events unchanged), so nothing here touched `~/.cache/delegate-graph/delegate-graph.db`. Raw transcript: `agent-output/settlement-convergence/live-load.log`.
