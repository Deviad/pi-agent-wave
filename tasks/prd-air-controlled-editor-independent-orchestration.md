...
- [x] US-005a (defect repair, added 2026-09-04): the schema-v5 migration retires pre-transport `agents.transport` vocabulary instead of failing closed on it. Every stored value outside `headless | herdr` observed in a real database (`delegate`, `opaque-delegate`) normalizes to `herdr` when the row carries either no ACPX/AgentFS provenance at all or a complete Herdr agent/tab/pane triple, so the row survives as a readable, non-focusable legacy agent with `presentation_identity: null`; a retired value combined with partial ACPX provenance still fails closed with the existing `agents table contains invalid transport presentation identity` error. Normalization is idempotent and preserves row count and all other columns. Proof is a seeded retired-vocabulary database in `extensions/pi-agent-wave/test/acpx-store-migration.test.ts` and the reopened real database reported in Implementation Status.
...
**Phase 1 attempt 2: schema-v5 migration fails closed on the live database (2026-09-04).**

Symptom: `/delegate` and every `delegate_graph` operation abort with `Error: agents table contains invalid transport presentation identity`, because `GraphStore` runs `migrateToV5()` on construction (`extensions/pi-agent-wave/store.ts:476`) and the live database at `~/.cache/delegate-graph/delegate-graph.db` is still at version 4.

Root cause (verified against the live database, not inferred): the table holds 294 agent rows; 292 are `transport='herdr'` and two are pre-transport vocabulary — `transport='delegate'` (`agent_bc61d4bc-…`, run `run_936dfcb6-…`) and `transport='opaque-delegate'` (`agent_a72d37c9-…`, run `run_c84d776c-…`), both created 2026-08-17 with every Herdr and ACPX column NULL. The v5 validity predicate accepts only `headless`/`herdr`, so those two rows reject the migration and the whole extension becomes unusable — one historical row blocks all new delegation. US-005 criterion 2 claimed existing databases reopen; this is the untested class behind that gap.

Repair: normalize retired vocabulary as specified in US-005a above. Mapping to `herdr` with an incomplete triple, rather than to `headless`, reuses the existing legacy carve-out in `projectAgent` (`store.ts:1180`), which yields `presentation_identity: null` — readable in `/graph status` and `/graph log`, not focusable and not reusable as a live attempt — and avoids synthesizing a headless ACPX-era identity for a row that predates ACPX. Failing closed is retained for genuinely contradictory rows.

Secondary finding, not fixed in this slice: `migrateToV5()` is called unconditionally on every construction, so each Pi session or worker that opens the store takes a `BEGIN IMMEDIATE` write transaction (`DROP`/`CREATE TRIGGER` plus the version row) even when the database is already at v5. With several Pi processes sharing the store and `busy_timeout=5000` (`store.ts:248`), that is the plausible source of the intermittent `database is locked` observed the same morning; separating genuine contention from that write-on-open pattern needs its own slice.

**US-005a verification (2026-09-04).**

- Failing test first: `node --experimental-strip-types --test extensions/pi-agent-wave/test/acpx-store-migration.test.ts` on the unmodified tree reproduced the production error verbatim — 3 of 13 subtests failed with `agents table contains invalid transport presentation identity`. After the repair, the same file reports `# pass 13 / # fail 0`, covering `delegate` and `opaque-delegate` normalization, idempotent reopen, row-and-column preservation via `deepEqual`, `presentation_identity: null` for normalized rows, retention of a complete Herdr triple under a retired label, and fail-closed behavior for a retired label hiding partial ACPX provenance.
- Repository gate: `node --experimental-strip-types --test extensions/pi-agent-wave/test/*.test.ts` → 373 tests, 360 pass, 11 skipped, 2 fail. Both failures are pre-existing at `6306474` and were confirmed by `git stash`-ing this change and re-running: `durable real ACPX report matrix evidence` (PRD blocker 2, ACPX 0.14.1 cleanup exit 1) and `production host audit bundle` (`ENOENT ... extensions/pi-agent-wave/extensions/pi-agent-wave`, cwd-dependent path in `scripts/production-audit.ts`). `npx tsc -p tsconfig.json` exits 0. The five Bun package checks pass (23/23).
- Real database: backed up to `~/.cache/delegate-graph/backups/pre-v5-20260904-181924/delegate-graph.db` (sha256 `59eca26906ad1f3398fa3d392e1ffc535f5eda3ad132811df79b7b9891c10943`), then opened with the patched store. Post-state read independently: `version=5`, `agents=294` (unchanged), retired-vocabulary rows `0`, both former rows now `transport='herdr'` with NULL identity columns. Reopening a second time reported `agents=294` again, so the migration is idempotent on real data.
- Write path rehearsed on a copy of that backup, never on the live store: `initRun` → `next` → `registerAgent` succeeded on the migrated database, the new Herdr agent projected `{kind:"herdr",agent:"agent-a",tabId:"tab-a",paneId:"pane-a"}`, and the normalized legacy `Thinker` row projected `null`. The live store still reported 39 runs with latest `created_at` 2026-08-31T15:37:29.337Z afterwards, confirming the rehearsal did not touch it.
- Live extension path: `delegate_graph op=status run_d1658e6b-9002-4e4d-bf2a-9ddf7290839e` returned the full agent table (5 workers, policy `Auto`, digest `ee0ee4cc…`) from the already-running Pi session, whose in-memory copy of `store.ts` predates this edit — proof that the data repair alone unblocks sessions started before the code fix. A fresh Pi session is still required to load the repaired migration for other legacy databases; the `/delegate` slash command itself has not been re-invoked by the user since the repair.
