...
Observed, and now traced (2026-09-09, same session): a serial run
(`--test-concurrency=1`) from the package directory gives 439 pass / **0 fail** / 11 skipped,
against 438 / 1 concurrently. The failing assertion's own payload names the cause:
...
cleanup scan. Twenty-four other test files start AgentFS, so under concurrent execution a
sibling's process is alive at the instant the scan runs. Confirmed by elimination as well as by
the payload: 70 two-file pairings of the audit file against every other test file produced
**zero** failures, and the file passes alone, which is why this is not attributable to one
poison sibling. It is also not leftover state — no AgentFS process and no leaked temporary
...
deliberately not quiet. Three ways out, none applied yet, and (a) and (c) touch product code:

- **(a) Inject the cleanup probe.** Let `runProductionAudit` take the machine probe as a
  parameter, defaulting to the real one. The suite then tests the gate logic against a
  synthetic quiet machine, and the real probe gets its own check. Makes the run deterministic
  in both directories and at any concurrency; keeps a production audit honest about the actual
  machine. Preferred.
- **(b) Serialize the file / the suite.** Cheapest, rejected: it hides a visible failure behind
  a run shape nobody types by hand, and with 24 possible contributors it is a permanent tax on
  every future parallel run.
- **(c) Scope the scan.** Count only processes whose command line references the audit's own
  workspace, so unrelated concurrency cannot trip it. Most precise, more work, and it changes
  what a production audit means by "clean".

Not counted as closed anywhere. Whichever route is taken, the acceptance question stays the
same: a full-suite run from either directory at default concurrency must report 0 failures.d cwd independence for package tests
