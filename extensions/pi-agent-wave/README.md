...
4. **Start a run.** From a Herdr workspace in your terminal the workers get tabs; from Air or a plain terminal they run headless:
   ...
   The run id is in the run notice and in `/graph status`. As soon as the first worker registers, a numbered agent list opens above the editor; press Enter to see the running worker's task, state, live output and retained answer in the terminal (with several running, Enter focuses the list and the arrows choose; a number then Enter also works), `q` goes back and then closes, Escape asks to cancel the run's workers, `/graph agents` reopens. The explicit follow view uses the same number-plus-Enter details and `q` closes it; `/graph focus` brings a worker's tab forward.
...
| `record` | Refused for every input; the error names `cancel`. Cancellation goes through `cancel`; every other transition was removed with the report contract. |
...
A settled `source_search` attempt is an **operational candidate**: the worker's answer, which ends with `VERDICT: DONE` or `VERDICT: BLOCKED`, plus the audited overlay changes under the owned paths (checkpoint, results, logs) staged as content. When a `checkpoint` was declared, the staged checkpoint file is parsed at settlement and its integer `jobsSaved` and string `status` are recorded as observed facts on the candidate and in the ledger; the process exit code is not observable by the supervisor and is not recorded. Artifacts reach the host through `integrate`, using the same journal as coding candidates with the Git checks switched off, so the working directory need not be a repository. `decide accepted` with `DONE` advances to synthesis; with `BLOCKED` the run parks `blocked`. Synthesis is accepted with `verdict: DONE`, which the supervisor supplies when the synthesis is complete (the thinker prompt does not demand a VERDICT line; the `collect` decision brief says so), and advances to the audit; any other verdict parks the run `blocked`. Every source result is ledgered automatically by operation ID in the derived run ledger, which covers every attempt, candidate, checkpoint and decision and is materialized to later workers as evidence. Concurrent source workers cannot own the same path or SQLite database; persist or merge shared data in a separate serialized stage.
...
When retries are exhausted, `retry` returns `exhausted: true` with the run parked in `awaiting_user` and emits a `runtime_retry_exhausted` progress event; no modal dialog, speech, or sound is raised, so headless and test runs never block on a desktop prompt. The operator resolves the parked run with `resolve` (`retry`, `defer`, `abort`, `escalate`) or `/graph resume`. A terminal picker for that choice (`resolveUserDecision`) exists in the code but is not wired to any command or tool path.
...
- **Adapters.** Pi is proven end to end on build, research and operations graphs. Codex is proven on research, build and operations. Claude has passed the probe but no graph run.
