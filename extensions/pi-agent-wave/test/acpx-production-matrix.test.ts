...
const RUN_REAL = process.env.RUN_REAL_ACPX_PRODUCTION_MATRIX === "1" && !!TOKEN_FILE && existsSync(TOKEN_FILE) && !!EVIDENCE_DIR;
...
function run(args: string[], env: NodeJS.ProcessEnv, timeout = 300_000) {
	return spawnSync(process.execPath, ["--experimental-strip-types", DELEGATE, "--transport", "herdr", "--", ...args], { cwd: ROOT, env, encoding: "utf8", timeout, shell: false });
}
...
function runAgent(agent: "pi" | "codex" | "claude", model: string): void {
	...
	const init = run(["init", `final-production-${agent}`], env);
	...
	assert.equal(settlement.identityMatches, true, "settlement must bind the re-planned Herdr identity, not the pending placeholder");
	assert.equal(settlement.presentationVerified, true);
	assert.equal(settlement.tabId, start.tab);
	assert.equal(settlement.herdrPaneId, start.pane);
	const cleanupBytes = readFileSync(wait.cleanupEvidencePath);
	const tabs = spawnSync("herdr", ["tab", "list", "--workspace", process.env.HERDR_WORKSPACE_ID ?? ""], { encoding: "utf8" });
	assert.equal(tabs.stdout.includes(start.tab), false);
	const evidence = {
		schemaVersion: 1,
	...
	const output = join(EVIDENCE_DIR!, `${agent}-production.json`);
...
describe("real production ACPX AgentFS Herdr matrix", () => {
