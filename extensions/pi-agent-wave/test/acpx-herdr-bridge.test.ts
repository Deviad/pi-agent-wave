...
// Owned paths arrive workspace-relative, so the helper must see the repository root as its
// working directory regardless of where the suite was started.
...
`, { mode: 0o755 });
		...
		const start = spawnSync("python3", [script, "start", runDir, "implementer", "coding", report, taskFile,
			...
			{ encoding: "utf8", cwd: repoRoot, env, timeout: 120_000 });
		...
		assert.deepEqual(resource.owned_paths, [fileURLToPath(new URL("../lib/acpx-types.ts", import.meta.url))]);
		...
		const transportSource = readFileSync(fileURLToPath(new URL("../scripts/delegate_core.py", import.meta.url)), "utf8");
		...
		assert.doesNotMatch(readFileSync(fileURLToPath(new URL("../scripts/acpx-worker.ts", import.meta.url)), "utf8"), /mode === "cancel"/);
		...
		const wait = spawnSync("python3", [script, "wait", runDir, result.agent], { encoding: "utf8", cwd: repoRoot, env, timeout: 120_000 });
