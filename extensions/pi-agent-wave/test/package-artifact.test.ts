...
describe("npm package artifact", () => {
	test("pack and publish dry-runs expose only package-owned runtime files", () => {
		...
		for (const required of ["package.json", "README.md", "LICENSE", "index.ts", "questionnaire.ts", "cmux-session.ts", "model-failover.ts", "claude-code-auth.ts", "lib/claude-auth-stream.ts", "lib/claude-auth-headers.ts", "lib/claude-auth-LICENSE", "require-runtime.ts", "require-acpx.ts", "require-agentfs.ts", "lib/jsonc.mjs", "lib/model-routing.mjs", "lib/model-failover-native.mjs", "lib/model-failover-native.d.mts", "lib/acpx-types.ts", "lib/acpx-events.ts", "lib/acpx-select.ts", "lib/acpx-permissions.ts", "lib/agentfs-sandbox.ts", "lib/worker-transport.ts", "scripts/delegate_core.py", "scripts/headless_delegate.py", "scripts/headless_supervisor.py", "scripts/herdr_delegate.py", "scripts/acpx-worker.ts", "scripts/acpx-cancel.ts", "scripts/acpx-plan.ts", "scripts/production-audit.ts", "scripts/production-secret-scan.ts", "scripts/production-cleanup-scan.ts", "scripts/production-token-cleanup.ts", "scripts/migrate.mjs", "scripts/policy-resolver.mjs", "scripts/resolve-model.mjs"]) {
